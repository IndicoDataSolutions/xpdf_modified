...
#ifndef JSONOUTPUTDEV_H
#define JSONOUTPUTDEV_H
...
// typedef void (*TextOutputFunc2)(void *stream, GString *s, int len);
...
class JSONTextFontInfo {
public:

  JSONTextFontInfo(GfxState *state);
  ~JSONTextFontInfo();
...
private:
  ...
  friend class JSONTextLine;
  friend class JSONTextPage;
  friend class JSONTextWord;
};
...
public:
  ...
  JSONTextWord(GList *chars, int start, int lenA,
	   int rotA, GBool spaceAfterA);
  ~JSONTextWord();
  JSONTextWord *copy() { return new JSONTextWord(this); }

  // Get the JSONTextFontInfo object associated with this word.
  JSONTextFontInfo *getFontInfo() { return font; }
...
private:
  ...
  JSONTextWord(JSONTextWord *word);
  void appendChar(JSONTextChar *ch);
  ...
  JSONTextFontInfo *font;		// font information
...
public:
  ...
  JSONTextLine(GList *wordsA, double xMinA, double yMinA,
...
private:
  ...
  friend class JSONTextPage;
  friend class JSONTextParagraph;
};
...
class JSONTextParagraph {
public:

  JSONTextParagraph(GList *linesA);
  ~JSONTextParagraph();

  // Get the list of JSONTextLine objects.
  GList *getLines() { return lines; }

private:

  GList *lines;			// [JSONTextLine]
  double xMin, xMax;		// bounding box x coordinates
  double yMin, yMax;		// bounding box y coordinates
...
class JSONTextColumn {
public:

  JSONTextColumn(GList *paragraphsA, double xMinA, double yMinA,
	     double xMaxA, double yMaxA);
  ~JSONTextColumn();

  // Get the list of JSONTextParagraph objects.
...
private:
  ...
  GList *paragraphs;		// [JSONTextParagraph]
  double xMin, xMax;		// bounding box x coordinates
  double yMin, yMax;		// bounding box y coordinates
...
class JSONTextWordList {
public:

  JSONTextWordList(GList *wordsA);
...
public:
  ...
  JSONTextPage(JSONTextOutputControl *controlA);
  ~JSONTextPage();
  ...
  // Create and return a list of JSONTextColumn objects.
  GList *makeColumns();

  // Get the list of all JSONTextFontInfo objects used on this page.
  GList *getFonts() { return fonts; }

  // Build a flat word list, in the specified ordering.
  JSONTextWordList *makeWordList();
...
private:
  ...
  JSONTextBlock *splitChars(GList *charsA);
  JSONTextBlock *split(GList *charsA, int rot);
  GList *getChars(GList *charsA, double xMin, double yMin,
		  double xMax, double yMax);
  void tagBlock(JSONTextBlock *blk);
  void insertLargeChars(GList *largeChars, JSONTextBlock *blk);
  void insertLargeCharsInFirstLeaf(GList *largeChars, JSONTextBlock *blk);
  void insertLargeCharInLeaf(JSONTextChar *ch, JSONTextBlock *blk);
  void insertIntoTree(JSONTextBlock *subtree, JSONTextBlock *primaryTree);
  void insertColumnIntoTree(JSONTextBlock *column, JSONTextBlock *tree);
  void insertClippedChars(GList *clippedChars, JSONTextBlock *tree);
  JSONTextBlock *findClippedCharLeaf(JSONTextChar *ch, JSONTextBlock *tree);
  GList *buildColumns(JSONTextBlock *tree);
  void buildColumns2(JSONTextBlock *blk, GList *columns);
  JSONTextColumn *buildColumn(JSONTextBlock *tree);
  double getLineIndent(JSONTextLine *line, JSONTextBlock *blk);
  double getAverageLineSpacing(GList *lines);
  double getLineSpacing(JSONTextLine *line0, JSONTextLine *line1);
  void buildLines(JSONTextBlock *blk, GList *lines);
  JSONTextLine *buildLine(JSONTextBlock *blk);
  void getLineChars(JSONTextBlock *blk, GList *charsA);
  ...
  void computeLinePhysWidth(JSONTextLine *line, UnicodeMap *uMap);
  ...
  void dumpTree(JSONTextBlock *tree, int indent = 0);
  ...
  int charPos;			// next character position (within content
				//   stream)
  JSONTextFontInfo *curFont;	// current font
...
public:
  ...
  // is maintained.  If <rawOrder> is true, the text is kept in
  // content stream order.
  JSONOutputDev(TextOutputFunc func, void *stream,
  ...
  // Returns the JSONTextPage object for the last rasterized page,
  // transferring ownership to the caller.
  JSONTextPage *takeText();
