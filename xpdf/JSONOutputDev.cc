...
// Table cells (JSONTextColumns) are allowed to overlap by this much
...
JSONTextChar::JSONTextChar(Unicode cA, int charPosA, int charLenA,
		   double xMinA, double yMinA, double xMaxA, double yMaxA,
		   int rotA, GBool clippedA, GBool invisibleA,
		   JSONTextFontInfo *fontA, double fontSizeA,
		   double colorRA, double colorGA, double colorBA) {
...
int JSONTextChar::cmpX(const void *p1, const void *p2) {
  const JSONTextChar *ch1 = *(const JSONTextChar **)p1;
  const JSONTextChar *ch2 = *(const JSONTextChar **)p2;
...
int JSONTextChar::cmpY(const void *p1, const void *p2) {
  const JSONTextChar *ch1 = *(const JSONTextChar **)p1;
  const JSONTextChar *ch2 = *(const JSONTextChar **)p2;
...
enum JSONTextBlockType {
  blkVertSplit,
  blkHorizSplit,
  blkLeaf
};

enum JSONTextBlockTag {
  blkTagMulticolumn,
  blkTagColumn,
  blkTagLine
};
...
public:
  ...
  JSONTextBlock(JSONTextBlockType typeA, int rotA);
  ~JSONTextBlock();
  void addChild(JSONTextBlock *child);
  void addChild(JSONTextChar *child);
  void prependChild(JSONTextChar *child);
  void updateBounds(int childIdx);

  JSONTextBlockType type;
  JSONTextBlockTag tag;
  int rot;
  double xMin, yMin, xMax, yMax;
  GBool smallSplit;		// true for blkVertSplit/blkHorizSplit
				//   where the gap size is small
  GList *children;		// for blkLeaf, children are JSONTextWord;
				//   for others, children are JSONTextBlock
};

JSONTextBlock::JSONTextBlock(JSONTextBlockType typeA, int rotA) {
  type = typeA;
...
JSONTextBlock::~JSONTextBlock() {
  if (type == blkLeaf) {
    delete children;
  } else {
    deleteGList(children, JSONTextBlock);
  }
}

void JSONTextBlock::addChild(JSONTextBlock *child) {
...
void JSONTextBlock::addChild(JSONTextChar *child) {
  if (children->getLength() == 0) {
    xMin = child->xMin;
    yMin = child->yMin;
    xMax = child->xMax;
...
void JSONTextBlock::prependChild(JSONTextChar *child) {
  if (children->getLength() == 0) {
    xMin = child->xMin;
    yMin = child->yMin;
    xMax = child->xMax;
...
void JSONTextBlock::updateBounds(int childIdx) {
  JSONTextBlock *child;

  child = (JSONTextBlock *)children->get(childIdx);
  if (child->xMin < xMin) {
    xMin = child->xMin;
  }
  if (child->yMin < yMin) {
...
class JSONTextUnderline {
public:

  JSONTextUnderline(double x0A, double y0A, double x1A, double y1A)
...
public:
  ...
  JSONTextLink(double xMinA, double yMinA, double xMaxA, double yMaxA,
	   GString *uriA)
...
//------------------------------------------------------------------------
// JSONTextOutputControl
//------------------------------------------------------------------------

JSONTextOutputControl::JSONTextOutputControl() {
...
JSONTextFontInfo::JSONTextFontInfo(GfxState *state) {
  GfxFont *gfxFont;
...
JSONTextFontInfo::~JSONTextFontInfo() {
  if (fontName) {
    delete fontName;
  }
}

GBool JSONTextFontInfo::matches(GfxState *state) {
  Ref *id;
...
// Build a JSONTextWord object, using chars[start .. start+len-1].
// (If rot >= 2, the chars list is in reverse order.)
JSONTextWord::JSONTextWord(GList *chars, int start, int lenA,
  ...
  case 0:
  default:
    ch = (JSONTextChar *)chars->get(start);
    xMin = ch->xMin;
    yMin = ch->yMin;
    yMax = ch->yMax;
    ch = (JSONTextChar *)chars->get(start + len - 1);
    xMax = ch->xMax;
    break;
  case 1:
    ch = (JSONTextChar *)chars->get(start);
  ...
  for (i = 0; i < len; ++i) {
    ch = (JSONTextChar *)chars->get(rot >= 2 ? start + len - 1 - i : start + i);
    text[i] = ch->c;
...
JSONTextWord::JSONTextWord(JSONTextWord *word) {
  *this = *word;
  text = (Unicode *)gmallocn(len, sizeof(Unicode));
...
void JSONTextWord::appendChar(JSONTextChar *ch) {
  if (ch->xMin < xMin) {
    xMin = ch->xMin;
  }
  if (ch->xMax > xMax) {
...
int JSONTextWord::cmpYX(const void *p1, const void *p2) {
  const JSONTextWord *word1 = *(const JSONTextWord **)p1;
  const JSONTextWord *word2 = *(const JSONTextWord **)p2;
  double cmp;
...
int JSONTextWord::cmpCharPos(const void *p1, const void *p2) {
  const JSONTextWord *word1 = *(const JSONTextWord **)p1;
  const JSONTextWord *word2 = *(const JSONTextWord **)p2;
...
void JSONTextWord::getCharBBox(int charIdx, double *xMinA, double *yMinA,
...
double JSONTextWord::getBaseline() {
  switch (rot) {
  case 0:
  default:
...
GString *JSONTextWord::getLinkURI() {
...
JSONTextLine::JSONTextLine(GList *wordsA, double xMinA, double yMinA,
...
JSONTextLine::~JSONTextLine() {
  deleteGList(words, JSONTextWord);
  gfree(text);
  gfree(edge);
}

double JSONTextLine::getBaseline() {
  JSONTextWord *word0;

  word0 = (JSONTextWord *)words->get(0);
  switch (rot) {
  case 0:
  default:
...
JSONTextParagraph::JSONTextParagraph(GList *linesA) {
  JSONTextLine *line;
  int i;
  ...
  // primaryLR = 1.;
  //
  // encodeFragment(text, n, uMap, primaryLR, s);

}

JSONTextParagraph::~JSONTextParagraph() {
  deleteGList(lines, JSONTextLine);
}
...
JSONTextColumn::JSONTextColumn(GList *paragraphsA, double xMinA, double yMinA,
		       double xMaxA, double yMaxA) {
...
JSONTextColumn::~JSONTextColumn() {
  deleteGList(paragraphs, JSONTextParagraph);
}

int JSONTextColumn::cmpX(const void *p1, const void *p2) {
  const JSONTextColumn *col1 = *(const JSONTextColumn **)p1;
  const JSONTextColumn *col2 = *(const JSONTextColumn **)p2;
...
int JSONTextColumn::cmpY(const void *p1, const void *p2) {
  const JSONTextColumn *col1 = *(const JSONTextColumn **)p1;
  const JSONTextColumn *col2 = *(const JSONTextColumn **)p2;
...
int JSONTextColumn::cmpPX(const void *p1, const void *p2) {
  const JSONTextColumn *col1 = *(const JSONTextColumn **)p1;
  const JSONTextColumn *col2 = *(const JSONTextColumn **)p2;
...
JSONTextWordList::JSONTextWordList(GList *wordsA) {
  words = wordsA;
}

JSONTextWordList::~JSONTextWordList() {
  deleteGList(words, JSONTextWord);
}

int JSONTextWordList::getLength() {
  return words->getLength();
}

JSONTextWord *JSONTextWordList::get(int idx) {
  if (idx < 0 || idx >= words->getLength()) {
    return NULL;
  }
  return (JSONTextWord *)words->get(idx);
}
...
JSONTextPage::JSONTextPage(JSONTextOutputControl *controlA) {
  control = *controlA;
...
JSONTextPage::~JSONTextPage() {
  clear();
  deleteGList(chars, JSONTextChar);
  deleteGList(fonts, JSONTextFontInfo);
  deleteGList(underlines, JSONTextUnderline);
  deleteGList(links, JSONTextLink);
  if (findCols) {
    deleteGList(findCols, JSONTextColumn);
  }
}

void JSONTextPage::startPage(GfxState *state) {
  clear();
  if (state) {
...
void JSONTextPage::updateFont(GfxState *state) {
  GfxFont *gfxFont;
  double *fm;
  char *name;
  ...
  for (i = 0; i < fonts->getLength(); ++i) {
    curFont = (JSONTextFontInfo *)fonts->get(i);
  ...
  if (!curFont) {
    curFont = new JSONTextFontInfo(state);
...
void JSONTextPage::addChar(GfxState *state, double x, double y,
		       double dx, double dy,
  ...
  // ActualText chars will be added by JSONTextPage::endActualText()).
  if (actualText) {
    if (!actualTextNBytes) {
      actualTextX0 = x;
      actualTextY0 = y;
  ...
  if (uLen > 0) {
	...
	j = i;
      ...
      chars->append(new JSONTextChar(u[j], charPos, nBytes, xMin, yMin, xMax, yMax,
				 curRot, clipped,
				 state->getRender() == 3,
				 curFont, curFontSize,
...
void JSONTextPage::incCharCount(int nChars) {
  charPos += nChars;
}

void JSONTextPage::beginActualText(GfxState *state, Unicode *u, int uLen) {
  if (actualText) {
    gfree(actualText);
  }
...
void JSONTextPage::endActualText(GfxState *state) {
  Unicode *u;

  u = actualText;
  actualText = NULL;  // so we can call JSONTextPage::addChar()
  if (actualTextNBytes) {
...
void JSONTextPage::addUnderline(double x0, double y0, double x1, double y1) {
  underlines->append(new JSONTextUnderline(x0, y0, x1, y1));
}

void JSONTextPage::addLink(double xMin, double yMin, double xMax, double yMax,
		       Link *link) {
  GString *uri;

  if (link && link->getAction() && link->getAction()->getKind() == actionURI) {
    uri = ((LinkURI *)link->getAction())->getURI()->copy();
    links->append(new JSONTextLink(xMin, yMin, xMax, yMax, uri));
  }
}
...
void JSONTextPage::write(void *outputStream, TextOutputFunc outputFunc) {
  UnicodeMap *uMap;
...
void JSONTextPage::writeReadingOrder(void *outputStream,
  ...
  for (colIdx = 0; colIdx < columns->getLength(); ++colIdx) {
    col = (JSONTextColumn *)columns->get(colIdx);
    for (parIdx = 0; parIdx < col->paragraphs->getLength(); ++parIdx) {
      par = (JSONTextParagraph *)col->paragraphs->get(parIdx);
      for (lineIdx = 0; lineIdx < par->lines->getLength(); ++lineIdx) {
	line = (JSONTextLine *)par->lines->get(lineIdx);
	n = line->len;
    ...
    j["parIdx"] = parIdx;
    j["lineIdx"] = lineIdx;
    j["fontSize"] = line->fontSize;
    j["hyphenated"] = line->hyphenated;
    bbox["xMin"] = line->xMin;
    bbox["yMin"] = line->yMin;
    bbox["xMax"] = line->xMax;
    bbox["yMax"] = line->yMax;
  ...
  std::string page_json_str = page_json.dump();
  (*outputFunc)(outputStream, page_json_str.c_str(), page_json_str.length());
  (*outputFunc)(outputStream, eol, eolLen);

  deleteGList(columns, JSONTextColumn);
}

GList *JSONTextPage::makeColumns() {
  JSONTextBlock *tree;
  GList *columns;
...
void JSONTextPage::writePhysLayout(void *outputStream,
			       TextOutputFunc outputFunc,
			       UnicodeMap *uMap,
			       char *space, int spaceLen,
			       char *eol, int eolLen) {
  ...
  columns->sort(&JSONTextColumn::cmpPX);
  for (colIdx = 0; colIdx < columns->getLength(); ++colIdx) {
    col = (JSONTextColumn *)columns->get(colIdx);
    y = col->py;
    for (parIdx = 0;
...
void JSONTextPage::writeLinePrinter(void *outputStream,
				TextOutputFunc outputFunc,
				UnicodeMap *uMap,
				char *space, int spaceLen,
				char *eol, int eolLen) {
  ...
  rot = rotateChars(chars);
  chars->sort(&JSONTextChar::cmpX);
  removeDuplicates(chars, 0);
  chars->sort(&JSONTextChar::cmpY);
  ...
  if (control.fixedPitch > 0) {
    ...
    for (i = 0; i < chars->getLength(); ++i) {
      ch = (JSONTextChar *)chars->get(i);
      for (j = i + 1; j < chars->getLength(); ++j) {
	ch2 = (JSONTextChar *)chars->get(j);
  ...
  if (chars->getLength()) {
    yMin0 = ((JSONTextChar *)chars->get(0))->yMin;
  ...
  for (y = yShift; y < pageHeight; y += lineSpacing) {
    ...
    line = new GList;
    while (i < chars->getLength() &&
	   ((JSONTextChar *)chars->get(i))->yMin < y + lineSpacing) {
      line->append(chars->get(i++));
    }
    line->sort(&JSONTextChar::cmpX);

    // shift the grid to avoid problems with floating point accuracy
    // -- for fixed char spacing, this avoids problems with
    // dropping/inserting spaces
    if (line->getLength()) {
      xMin0 = ((JSONTextChar *)line->get(0))->xMin;
...
void JSONTextPage::writeRaw(void *outputStream,
			TextOutputFunc outputFunc,
			UnicodeMap *uMap,
			char *space, int spaceLen,
			char *eol, int eolLen) {
  ...
  for (i = 0; i < chars->getLength(); ++i) {
    ...
    if (i+1 < chars->getLength()) {
      ch2 = (JSONTextChar *)chars->get(i+1);
      if (ch2->rot != ch->rot) {
	s->append(eol, eolLen);
      } else {
	switch (ch->rot) {
...
void JSONTextPage::encodeFragment(Unicode *text, int len, UnicodeMap *uMap,
...
// Rotate the JSONTextUnderlines and JSONTextLinks to match the transform
// performed by rotateChars().
void JSONTextPage::rotateUnderlinesAndLinks(int rot) {
  JSONTextUnderline *underline;
  JSONTextLink *link;
  double xMin, yMin, xMax, yMax;
  int i;
  ...
  case 1:
    for (i = 0; i < underlines->getLength(); ++i) {
      underline = (JSONTextUnderline *)underlines->get(i);
      xMin = underline->y0;
      xMax = underline->y1;
...
void JSONTextPage::unrotateChars(GList *charsA, int rot) {
  JSONTextChar *ch;
  double xMin, yMin, xMax, yMax, t;
  int i;
...
// Undo the coordinate transform performed by rotateChars().
void JSONTextPage::unrotateColumns(GList *columns, int rot) {
  JSONTextColumn *col;
  JSONTextParagraph *par;
  JSONTextLine *line;
  JSONTextWord *word;
  ...
  case 1:
	...
	for (lineIdx = 0;
	  ...
	  for (wordIdx = 0; wordIdx < line->words->getLength(); ++wordIdx) {
	    word = (JSONTextWord *)line->words->get(wordIdx);
...
void JSONTextPage::unrotateWords(GList *words, int rot) {
  JSONTextWord *word;
  double xMin, yMin, xMax, yMax;
  int i, j;
...
// LR, false for RL.
GBool JSONTextPage::checkPrimaryLR(GList *charsA) {
  JSONTextChar *ch;
  int i, lrCount;
...
void JSONTextPage::removeDuplicates(GList *charsA, int rot) {
  JSONTextChar *ch, *ch2;
...
// Split the characters into trees of JSONTextBlocks, one tree for each
// rotation.  Merge into a single tree (with the primary rotation).
JSONTextBlock *JSONTextPage::splitChars(GList *charsA) {
  JSONTextBlock *tree[4];
  JSONTextBlock *blk;
  GList *chars2, *clippedChars;
  JSONTextChar *ch;
  int rot, i;

  // split: build a tree of JSONTextBlocks for each rotation
  ...
  for (rot = 0; rot < 4; ++rot) {
    ...
    if (chars2->getLength() > 0) {
      chars2->sort((rot & 1) ? &JSONTextChar::cmpY : &JSONTextChar::cmpX);
      removeDuplicates(chars2, rot);
      if (control.clipText) {
	i = 0;
	while (i < chars2->getLength()) {
	  ch = (JSONTextChar *)chars2->get(i);
	  if (ch->clipped) {
	    ch = (JSONTextChar *)chars2->del(i);
  ...
  if (tree[0]->tag != blkTagMulticolumn) {
    blk = new JSONTextBlock(blkHorizSplit, 0);
    blk->addChild(tree[0]);
...
// Generate a tree of JSONTextBlocks, marked as columns, lines, and words.
JSONTextBlock *JSONTextPage::split(GList *charsA, int rot) {
  JSONTextBlock *blk;
  GList *chars2, *chars3;
  ...
  // split vertically
  if (doVertSplit) {
    blk = new JSONTextBlock(blkVertSplit, rot);
  ...
  // split horizontally
    ...
    blk = new JSONTextBlock(blkHorizSplit, rot);
    blk->smallSplit = smallSplit;
    for (start = yMinI;
  ...
  // create a leaf node
    ...
    blk = new JSONTextBlock(blkLeaf, rot);
    for (i = 0; i < charsA->getLength(); ++i) {
      blk->addChild((JSONTextChar *)charsA->get(i));
    }
  }
...
GList *JSONTextPage::getChars(GList *charsA, double xMin, double yMin,
...
void JSONTextPage::tagBlock(JSONTextBlock *blk) {
  JSONTextBlock *child;
  int i;
  ...
  if (control.mode == textOutTableLayout) {
	...
	child = (JSONTextBlock *)blk->children->get(i);
...
void JSONTextPage::insertLargeChars(GList *largeChars, JSONTextBlock *blk) {
  JSONTextChar *ch, *ch2;
  GBool singleLine;
  ...
  for (i = 0; i < largeChars->getLength(); ++i) {
    ch2 = (JSONTextChar *)largeChars->get(i);
    if (ch2->xMax > xLimit || ch2->yMax > yLimit) {
      singleLine = gFalse;
      break;
    }
    if (i > 0) {
      ch = (JSONTextChar *)largeChars->get(i-1);
  ...
  if (singleLine) {
    ...
    for (i = largeChars->getLength() - 1; i >= 0; --i) {
      ch = (JSONTextChar *)largeChars->get(i);
...
// list of large chars.
void JSONTextPage::insertLargeCharsInFirstLeaf(GList *largeChars, JSONTextBlock *blk) {
  JSONTextChar *ch;
  int i;
  ...
  if (blk->type == blkLeaf) {
    ...
    insertLargeCharsInFirstLeaf(largeChars, (JSONTextBlock *)blk->children->get(0));
    blk->updateBounds(0);
  }
}
...
// it.
void JSONTextPage::insertLargeCharInLeaf(JSONTextChar *ch, JSONTextBlock *blk) {
  JSONTextBlock *child;
  double y;
  int i;
  ...
  if (blk->type == blkLeaf) {
    ...
    insertLargeCharInLeaf(ch, (JSONTextBlock *)blk->children->get(0));
    blk->updateBounds(0);
  }
}

// Merge blk (rot != 0) into primaryTree (rot == 0).
void JSONTextPage::insertIntoTree(JSONTextBlock *blk, JSONTextBlock *primaryTree) {
  JSONTextBlock *child;
  ...
  if (blk->tag == blkTagMulticolumn) {
    while (blk->children->getLength()) {
      child = (JSONTextBlock *)blk->children->del(0);
...
void JSONTextPage::insertColumnIntoTree(JSONTextBlock *column, JSONTextBlock *tree) {
  JSONTextBlock *child;
  int i;

  for (i = 0; i < tree->children->getLength(); ++i) {
    child = (JSONTextBlock *)tree->children->get(i);
...
// Insert clipped characters back into the JSONTextBlock tree.
void JSONTextPage::insertClippedChars(GList *clippedChars, JSONTextBlock *tree) {
  JSONTextChar *ch, *ch2;
  JSONTextBlock *leaf;
  double y;
  int i;
  ...
  clippedChars->sort(JSONTextChar::cmpX);
  while (clippedChars->getLength()) {
    ch = (JSONTextChar *)clippedChars->del(0);
    if (ch->rot != 0) {
      continue;
    }
...
JSONTextBlock *JSONTextPage::findClippedCharLeaf(JSONTextChar *ch, JSONTextBlock *tree) {
  JSONTextBlock *ret, *child;
  double y;
  int i;
...
// Convert the tree of JSONTextBlocks into a list of JSONTextColumns.
GList *JSONTextPage::buildColumns(JSONTextBlock *tree) {
  GList *columns;
...
void JSONTextPage::buildColumns2(JSONTextBlock *blk, GList *columns) {
  JSONTextColumn *col;
  int i;
  ...
  case blkTagMulticolumn:
    for (i = 0; i < blk->children->getLength(); ++i) {
      buildColumns2((JSONTextBlock *)blk->children->get(i), columns);
    }
    break;
  }
}

JSONTextColumn *JSONTextPage::buildColumn(JSONTextBlock *blk) {
  GList *lines, *parLines;
  GList *paragraphs;
  JSONTextLine *line0, *line1;
  ...
  while (i < lines->getLength()) {
    ...
    parLines = new GList();
    line0 = (JSONTextLine *)lines->get(i);
    parLines->append(line0);
    ++i;


    if (i < lines->getLength()) {
      line1 = (JSONTextLine *)lines->get(i);
	...
	for (++i; i < lines->getLength(); ++i) {
	  ...
	  if (getLineSpacing((JSONTextLine *)lines->get(i - 1), line1)
	        > spaceThresh) {
	    break;
	  }
	  parLines->append(line1);
    ...
    paragraphs->append(new JSONTextParagraph(parLines));
  }

  delete lines;

  return new JSONTextColumn(paragraphs, blk->xMin, blk->yMin,
			blk->xMax, blk->yMax);
}

double JSONTextPage::getLineIndent(JSONTextLine *line, JSONTextBlock *blk) {
  double indent;
...
double JSONTextPage::getAverageLineSpacing(GList *lines) {
  double avg, sp;
  int n, i;
  ...
  for (i = 1; i < lines->getLength(); ++i) {
    sp = getLineSpacing((JSONTextLine *)lines->get(i - 1),
			(JSONTextLine *)lines->get(i));
    if (sp > 0) {
      avg += sp;
      ++n;
    }
...
double JSONTextPage::getLineSpacing(JSONTextLine *line0, JSONTextLine *line1) {
  double sp;
...
void JSONTextPage::buildLines(JSONTextBlock *blk, GList *lines) {
  JSONTextLine *line;
  int i;
  ...
  case blkTagMulticolumn: // multicolumn should never happen here
    for (i = 0; i < blk->children->getLength(); ++i) {
      buildLines((JSONTextBlock *)blk->children->get(i), lines);
    }
    break;
  }
}

JSONTextLine *JSONTextPage::buildLine(JSONTextBlock *blk) {
  GList *charsA;
  GList *words;
  JSONTextChar *ch, *ch2;
  JSONTextWord *word;
  ...
  while (i < charsA->getLength()) {
    ...
    for (j = i+1; j < charsA->getLength(); ++j) {
      ch = (JSONTextChar *)charsA->get(j-1);
      ch2 = (JSONTextChar *)charsA->get(j);
    ...
    word = new JSONTextWord(charsA, i, j - i, blk->rot,
  ...
  return new JSONTextLine(words, blk->xMin, blk->yMin, blk->xMax, blk->yMax,
		      lineFontSize);
}

void JSONTextPage::getLineChars(JSONTextBlock *blk, GList *charsA) {
  int i;
  ...
  if (blk->type == blkLeaf) {
    ...
    for (i = 0; i < blk->children->getLength(); ++i) {
      getLineChars((JSONTextBlock *)blk->children->get(i), charsA);
    }
  }
}
...
// spaces.
double JSONTextPage::computeWordSpacingThreshold(GList *charsA, int rot) {
  JSONTextChar *ch, *ch2;
  ...
  for (i = 0; i < charsA->getLength(); ++i) {
    ...
    if (i < charsA->getLength() - 1) {
      ch2 = (JSONTextChar *)charsA->get(i+1);
...
int JSONTextPage::assignPhysLayoutPositions(GList *columns) {
  assignLinePhysPositions(columns);
  return assignColumnPhysPositions(columns);
}

// Assign a physical x coordinate for each JSONTextLine (relative to the
// containing JSONTextColumn).  This also computes JSONTextColumn width and
// height.
void JSONTextPage::assignLinePhysPositions(GList *columns) {
  JSONTextColumn *col;
  JSONTextParagraph *par;
  JSONTextLine *line;
  UnicodeMap *uMap;
...
void JSONTextPage::computeLinePhysWidth(JSONTextLine *line, UnicodeMap *uMap) {
  char buf[8];
  int n, i;
...
// Assign physical x and y coordinates for each JSONTextColumn.  Returns
// the text height (max physical y + 1).
int JSONTextPage::assignColumnPhysPositions(GList *columns) {
  JSONTextColumn *col, *col2;
  ...
  // assign x positions
  columns->sort(&JSONTextColumn::cmpX);
  for (i = 0; i < columns->getLength(); ++i) {
    col = (JSONTextColumn *)columns->get(i);
    if (control.fixedPitch) {
      col->px = (int)(col->xMin / control.fixedPitch);
    } else {
      col->px = 0;
      for (j = 0; j < i; ++j) {
	col2 = (JSONTextColumn *)columns->get(j);
  ...
  // assign y positions
  ph = 0;
  columns->sort(&JSONTextColumn::cmpY);
  for (i = 0; i < columns->getLength(); ++i) {
    col = (JSONTextColumn *)columns->get(i);
    col->py = 0;
    for (j = 0; j < i; ++j) {
...
void JSONTextPage::generateUnderlinesAndLinks(GList *columns) {
  JSONTextColumn *col;
  JSONTextParagraph *par;
  JSONTextLine *line;
  JSONTextWord *word;
...
GBool JSONTextPage::findText(Unicode *s, int len,
  ...
  for (colIdx = backward ? findCols->getLength() - 1 : 0;
       ...
       colIdx += backward ? -1 : 1) {
    column = (JSONTextColumn *)findCols->get(colIdx);
    ...
    for (parIdx = backward ? column->paragraphs->getLength() - 1 : 0;
	 ...
	 parIdx += backward ? -1 : 1) {
      par = (JSONTextParagraph *)column->paragraphs->get(parIdx);
...
GString *JSONTextPage::getText(double xMin, double yMin,
			   double xMax, double yMax) {
  UnicodeMap *uMap;
  char space[8], eol[16];
  int spaceLen, eolLen;
...
GBool JSONTextPage::findCharRange(int pos, int length,
			      double *xMin, double *yMin,
			      double *xMax, double *yMax) {
  JSONTextChar *ch;
...
JSONTextWordList *JSONTextPage::makeWordList() {
  JSONTextBlock *tree;
  GList *columns;
  JSONTextColumn *col;
  JSONTextParagraph *par;
  ...
  if (!tree) {
    // no text
    unrotateChars(chars, rot);
    return new JSONTextWordList(new GList());
  }
  columns = buildColumns(tree);
  delete tree;
  unrotateChars(chars, rot);
  ...
  case textOutTableLayout:
  case textOutLinePrinter:
    words->sort(&JSONTextWord::cmpYX);
    break;
  case textOutRawOrder:
    words->sort(&JSONTextWord::cmpCharPos);
    break;
  }
  ...
  return new JSONTextWordList(words);
}
...
void JSONTextPage::dumpChars(GList *charsA) {
  JSONTextChar *ch;
  int i;
...
void JSONTextPage::dumpTree(JSONTextBlock *tree, int indent) {
  JSONTextChar *ch;
  int i;
  ...
  if (tree->type == blkLeaf) {
    for (i = 0; i < tree->children->getLength(); ++i) {
      ch = (JSONTextChar *)tree->children->get(i);
    ...
    for (i = 0; i < tree->children->getLength(); ++i) {
      dumpTree((JSONTextBlock *)tree->children->get(i), indent + 2);
    }
  }
}

void JSONTextPage::dumpColumns(GList *columns) {
  JSONTextColumn *col;
  JSONTextParagraph *par;
  JSONTextLine *line;
...
JSONOutputDev::JSONOutputDev(char *fileName, char *pathFileName, JSONTextOutputControl *controlA,
  ...
  // set up text object
  text = new JSONTextPage(&control);
}

JSONOutputDev::JSONOutputDev(TextOutputFunc func, void *stream,
			     JSONTextOutputControl *controlA) {
  outputFunc = func;
  outputStream = stream;
  needClose = gFalse;
  control = *controlA;
...
void JSONOutputDev::beginString(GfxState *state, GString *s) {
}
...
void JSONOutputDev::incCharCount(int nChars) {
  text->incCharCount(nChars);
}

void JSONOutputDev::beginActualText(GfxState *state, Unicode *u, int uLen) {
  text->beginActualText(state, u, uLen);
}

void JSONOutputDev::endActualText(GfxState *state) {
  text->endActualText(state);
}

void JSONOutputDev::stroke(GfxState *state) {
  GfxPath *path;
  GfxSubpath *subpath;
  double x[2], y[2];
...
void JSONOutputDev::fill(GfxState *state) {
  GfxPath *path;
  GfxSubpath *subpath;
  double x[5], y[5];
...
void JSONOutputDev::eoFill(GfxState *state) {
  if (!control.html) {
    return;
  }
  fill(state);
...
void JSONOutputDev::processLink(Link *link) {
  double x1, y1, x2, y2;
...
GString *JSONOutputDev::getText(double xMin, double yMin,
				double xMax, double yMax) {
  return text->getText(xMin, yMin, xMax, yMax);
}

GBool JSONOutputDev::findCharRange(int pos, int length,
				   double *xMin, double *yMin,
				   double *xMax, double *yMax) {
  return text->findCharRange(pos, length, xMin, yMin, xMax, yMax);
}

JSONTextWordList *JSONOutputDev::makeWordList() {
  return text->makeWordList();
}

JSONTextPage *JSONOutputDev::takeText() {
  JSONTextPage *ret;
